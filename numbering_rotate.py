...
def normalize_rotation(pdf_bytes: bytes) -> bytes:
    """
    コピー機スキャンPDFの /Rotate 属性をコンテンツに焼き込み、
    PDFリーダーの「印刷→PDF」と同等の正規化を行う。
    """
    src = fitz.open(stream=pdf_bytes, filetype="pdf")
    dst = fitz.open()

    for page in src:
        # page.rect は /Rotate を考慮した「表示上の」矩形を返す
        rect = page.rect
        new_page = dst.new_page(width=rect.width, height=rect.height)
        # show_pdf_page がソースページを回転適用済みで描画 → 回転が焼き込まれる
